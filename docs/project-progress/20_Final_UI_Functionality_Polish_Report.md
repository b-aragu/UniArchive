...
2. **Upload Duplicate Warning Not Displaying**
   - **Issue:** The upload component successfully triggered the duplicate detection pipeline on the backend, but the frontend did not display the returned `duplicate_warning` payload.
   - **Fix:** Added conditional UI logic in `UploadPage.tsx` to render a highly visible warning block indicating the presence of a duplicate and noting it is flagged for review. Added a direct "View Document" link to navigate immediately to the uploaded resource.
